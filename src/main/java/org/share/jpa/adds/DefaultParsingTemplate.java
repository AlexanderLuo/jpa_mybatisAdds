...
import org.share.develop.io.ClassLoaderWrapper;
import org.share.develop.io.Resources;
import org.share.jpa.adds.parser.XNode;
import org.share.jpa.adds.parser.XPathParser;
import org.share.jpa.adds.scripting.*;
...
public class DefaultParsingTemplate implements ParsingTemplate{
    ...
    @Override
    public  void parse(String path) throws Throwable{
        this.path = path;
        InputStream inputStream = load(path);
        XNode root = resolveMapperNode(inputStream);
        inputStream.close();

        Class<?>  mapperInterface = Class.forName(root.getStringAttribute("namespace"));

        List<XNode>  sqls = resolveSQLNode(root);
    ...
    @Override
    public XNode resolveMapperNode(InputStream inputStream) throws IOException {
        XPathParser parser = new XPathParser(inputStream, false, null, null);
        XNode root = parser.evalNode("/mapper");
        if (StringUtils.isEmpty(root.getStringAttribute("namespace")))
            throw  new RuntimeException(this.path + "no namespace node");
        return  root;

    }

    @Override
    public List<XNode> resolveSQLNode(XNode root) {
